...
mkdir -p "$HOME/.local/bin" "$HOME/.local/opt" "$HOME/.cache/edgeai_ev_charger_monitor_demo"
...
if ! python3 -m pip -V >/dev/null 2>&1; then
  ...
  GETPIP="$HOME/.cache/edgeai_ev_charger_monitor_demo/get-pip.py"
...
cache = os.path.join(home, ".cache", "edgeai_ev_charger_monitor_demo")
